...
def item_price_dic_f(branchID, companyID, item_orders_raw):
    ...
    selected_company = item_orders_raw[(item_orders_raw['BranchID'] == branchID) & (item_orders_raw['CompanyID'] == companyID)]
    selected_company['ItemID'] = selected_company['ItemID'].astype(str)
    item_price = dict(zip(selected_company['ItemID'], selected_company['ItemPrice']))
    return item_price
...
def read_data():
    # df = read_excel('miniorder1.xlsx')
    df = pd.read_excel('miniorder1.xlsx', engine='openpyxl',)
    # print(df)
    return df
...
def main(company_id = 16, branch_id = 23, table_name = 'TESTING_TONGTBL'):
    ...
    item_orders_raw = read_data()
    company_order = pd.DataFrame()
    col_generate = ['OrderID', 'ItemID',  'BranchID']
    print("import data")

    # get data set with specific companyID and branchID
    for col in col_generate:
        print("company_ID", company_id)
        company_order[col] = item_orders_raw[item_orders_raw['CompanyID'] == company_id][col]
    train_data = company_order[company_order['BranchID'] == branch_id]
    train_data.drop(labels=['BranchID'], axis=1, inplace=True)
    ...
    print("order_item_dic", order_item_dic)
    print("order_item_list", order_items_list)
    ...
    return list(item_order_dic.values()), item_price, order_items_list
